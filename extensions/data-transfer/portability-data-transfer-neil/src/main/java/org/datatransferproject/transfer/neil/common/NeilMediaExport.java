...
public class NeilMediaExport {

  protected final NeilClient neilClient;
  ...
  public NeilMediaExport(NeilClient neilClient, Monitor monitor) {
    this.neilClient = neilClient;
  ...
  public void export() throws IOException, InvalidTokenException {
    ...
    rootPath = neilClient.getRootPath();
    items = neilClient.listRecursive(rootPath);
  ...
  protected String getFetchableUrl(String fullPath) throws IOException, InvalidTokenException {
    try {
      return neilClient.fileLink(fullPath);
    } catch (IOException e) {
      monitor.severe(() -> String.format("Neil file link error: %s", fullPath), e);
      return null;
    }
  }
